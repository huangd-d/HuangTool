...
contextBridge.exposeInMainWorld('electronAPI', {
  ...
  // MySQL 管理
  mysqlConnect: (config) => ipcRenderer.invoke('mysql-connect', config),
  mysqlDisconnect: (connectionId) => ipcRenderer.invoke('mysql-disconnect', connectionId),
  mysqlListDatabases: (connectionId) => ipcRenderer.invoke('mysql-list-databases', connectionId),
  mysqlListTables: (connectionId, database) => ipcRenderer.invoke('mysql-list-tables', connectionId, database),
  mysqlExecuteQuery: (connectionId, database, sql) => ipcRenderer.invoke('mysql-execute-query', connectionId, database, sql),
  mysqlGetConnections: () => ipcRenderer.invoke('mysql-get-connections'),
  mysqlGetSavedConnections: () => ipcRenderer.invoke('mysql-get-saved-connections'),
  mysqlSaveConnection: (config) => ipcRenderer.invoke('mysql-save-connection', config),
  mysqlDeleteSavedConnection: (id) => ipcRenderer.invoke('mysql-delete-saved-connection', id),
  mysqlTestConnection: (config) => ipcRenderer.invoke('mysql-test-connection', config),
  mysqlCreateDatabase: (connectionId, databaseName, charset, collation) => ipcRenderer.invoke('mysql-create-database', connectionId, databaseName, charset, collation),
  mysqlDropDatabase: (connectionId, databaseName) => ipcRenderer.invoke('mysql-drop-database', connectionId, databaseName),
  mysqlCreateTable: (connectionId, database, tableName, columns) => ipcRenderer.invoke('mysql-create-table', connectionId, database, tableName, columns),
  mysqlDropTable: (connectionId, database, tableName) => ipcRenderer.invoke('mysql-drop-table', connectionId, database, tableName),
  mysqlGetTableStructure: (connectionId, database, tableName) => ipcRenderer.invoke('mysql-get-table-structure', connectionId, database, tableName),
  mysqlAlterTableAddColumn: (connectionId, database, tableName, column) => ipcRenderer.invoke('mysql-alter-table-add-column', connectionId, database, tableName, column),
  // 窗口控制
